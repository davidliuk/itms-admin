      ...
      <a-row>
        ...
        <a-col :flex="'86px'" style="text-align: right">
          <a-space direction="vertical" :size="18">
            <a-button type="primary" @click="searchSettlementBy(formModel.id,formModel.userName)">
              <template #icon>
                <icon-search />
              </template>
      ...
      >
        ...
        <template #operations="{ record }">
          <a-button
              ...
              type="text"
              size="small"
              @click="searchSettlementById(record.id)"
          >
            {{ $t('settlement.columns.operations.detail') }}
          </a-button>
        </template>
        <!-- table里 -->
        <!-- 删改 -->
...
import {
  querySettlementList,
  Settlement,
  querySettlementListById, querySettlementListByUId, searchSettlementList
} from '@/api/finance';
