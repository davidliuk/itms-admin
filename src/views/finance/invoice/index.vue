      ...
      <a-row>
        <a-col :flex="1">
          ...
          >
            <a-row :gutter="16">
              ...
              <a-col :span="8">
                <a-form-item field="name" :label="$t('invoice.form.name')">
                  <a-input
                    v-model="formModel.name"
                    :placeholder="$t('invoice.form.name.placeholder')"
                  />
                </a-form-item>
              </a-col>
        ...
        <a-col :flex="'86px'" style="text-align: right">
          <a-space direction="vertical" :size="18">
            <a-button type="primary" @click="searchInvoiceById(formModel.id)">
              <template #icon>
                <icon-search />
              </template>
...
<script lang="ts" setup>
  ...
  import {queryInvoiceList, addInvoice, deleteInvoice, updateInvoice, Invoice, getInvoiceById} from '@/api/finance';
  ...
  const statusOptions = computed<SelectOptionData[]>(() => [
    {
      label: t('invoice.form.status.online'),
      value: 'online',
    },
    {
      label: t('invoice.form.status.offline'),
      value: 'offline',
    },
  ]);
  ...
  const searchInvoiceById = async (id: number) => {
    setLoading(true);
    try {
